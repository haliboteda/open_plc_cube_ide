...
**状态：方向已定，未实现（2026-08-16）。** 配图版见 artifact「板子归谁」
...
| | 影响 |
|---|---|
| 保留 4K / 8K / 16K | 记录条数 + 给证书验证代码留多少余量。撤销状态也要住进来 |
| 要不要上 WRP | 唯一能真正禁止 app 写这片区域的手段。H7 选项字节改动需解锁并复位 |
| 认领时要不要绑 UID | 防止把一块板的记录整段搬到另一块板 |
| 恢复出厂怎么触发 | **BOOT0 长按已被「进上传模式」占用**，需要一个能区分开的动作 |
