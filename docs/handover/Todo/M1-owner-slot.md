...
⚠️ **第 6 步的清除动作必须放在 `server_decide()` 之前。** `owner_slot_report()` 在它里面打日志；清除放在后面的话，**这一次启动的日志会先报旧的所有权、然后才把它清掉** —— 而那恰恰是唯一一次肯定有人在读日志的启动。接缝已经挪到正确位置（[main.c](../../../Core/Src/main.c) 里那个 `if (gesture == BOOT0_GESTURE_FACTORY_RESET)` 块），第 6 步填进去即可。
...
按 10 秒之前必须先做，否则**就是 3V3 对地短路 10 秒**。已做完并用受控实验验证，完整记录在 [../../HARDWARE-FACTS.md](../../HARDWARE-FACTS.md) —— 顺带否定了"改 input 会让 RESET 失效"那条挂了很久的怀疑（6 次物理复位全正常，BOOT0 也读得到）。
