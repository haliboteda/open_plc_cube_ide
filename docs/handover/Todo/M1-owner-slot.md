...
⚠️ **现在代码里没有"时长"这个概念。** [main.c](../../../Core/Src/main.c) 的 `boot_window_relay()` 是：继电器响 3×500ms **纯等待**，然后 `boot0_is_pressed()` **只读一次**。判据是"在 t=1.5s 那一瞬手是不是按着"，不是"按够 1.5 秒"。要区分 1.5s 和 10s 得改成**轮询计时**，那是新增逻辑不是调参数。
