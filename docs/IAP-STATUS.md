...
### SDRAM 数据线 D1（`PD15`）不可靠 —— 2026-08-21 实测，T0 从通过变失败

**症状**：`** SDRAM SELF-TEST FAILED at offset 00000000 **`，三次复位三次一样，100% 复现。T0 在 2026-08-18 是通过的。

**结论：D1 这一根线上是开路/接触不良，位置在 MCU `PD15` 与 SDRAM（AS4C32M16SB-7BIN）D1 脚之间。** 不是软件。证据链：

| 观察 | 说明 |
|---|---|
| 每一处损坏都恰好是 `0x00020002` | 总线是 16 位（`Core/Src/fmc.c` `FMC_SDRAM_MEM_BUS_WIDTH_16`），一次 32 位访问是两次 16 位传输，**两个半字的 bit1 是同一根物理线** |
| 那根线是 `FMC_D1` = `PD15` | `Core/Src/fmc.c` 的 `HAL_FMC_MspInit()`；[HARDWARE-FACTS.md](HARDWARE-FACTS.md) 的 D0–D15 表 |
| 写 `0xFFFFFFFF` 读回 `FFFDFFFD`；写 `0x12345678` 曾读回 `1236567A` | **两个方向都错** → 不是固定拉高或拉低 |
| 同一模式有时又是对的 | 间歇性 —— 悬空线的形状 |
| CPU 全速访问必错，SWD 慢速访问多数正确 | 同上 |
| 源码自 T0 通过起一行未动 | `git log --since=2026-08-18 -- Core/ IAPServer/ Drivers/ …` 为空 |
| 冷启动（`Reset cause: POR`）照样失败 | 不是上一个 app 把芯片留在坏状态 |
| SDCLK 降到 HCLK/3 仍失败 | 排除边缘时序。⚠️ 该实验有混淆：刷新计数 762 是按 100 MHz 硬算的，降频时没跟着改（应为 ~501）。但**刷新过慢导致的是毫秒级衰减，而自检写完几微秒就读回**，所以不影响这条结论 |
| `PD15` 完全跟随内部上拉/下拉（上拉读 1、下拉读 0） | **MCU 引脚没坏，外部也没有东西拉着它**（内部上拉仅约 40 kΩ，任何真实驱动或短路都会赢）→ 故障在 `PD15` 通往 SDRAM 的那一段 |

**开路完美解释全部现象**：写的时候 MCU 驱动 D1 而 SDRAM 收不到，存进去的是悬空电平；读的时候 SDRAM 驱动 D1 而 MCU 收不到，读到的是悬空电平 —— 两个方向都错、且偶尔碰巧对。

**影响**：`Core/Src/fmc.c` 的注释说得对 —— 坏 SDRAM 里暂存的镜像会**校验失败，app 区根本不被碰**，板子仍然安全。但**任何上传都停在 checksum**，所以 **S4a / S4b 在这块板上跑不了**（它们要的就是"传输中"和"擦写中"两个窗口）。

**顺带一条正面结论：D4 这条需求（启动自检 SDRAM 并把根因打出来）今天证明了自己的价值。** 没有它，这件事的表现会是"上传总是 Checksum Failed"，排查方向完全不同。
