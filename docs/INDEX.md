...
- **★ 一个事实只写在一个文件里。** 数字 → [test/MEASUREMENTS.md](test/MEASUREMENTS.md)；状态 → [STATUS.md](STATUS.md)；判据 → `TEST-CASES.md`；硬件事实 → [design/HARDWARE-FACTS.md](design/HARDWARE-FACTS.md)；已拍板的 → [design/DECISIONS.md](design/DECISIONS.md)。**别处只留一行指针，不抄内容。**

  ⚠️ 2026-08-22 清理时，同一条需求的状态在**五个**文件里各写一遍，三个数字对不上 —— 而它们看起来一模一样。当天找出并消掉了 **19 段重复散文**。

  **这条现在有东西盯着**：用例 **P8**（`$TOOL:TestTool/tools/check_doc_dupes.py`，在 selfcheck 里）把每份文档切成句子，**同一句出现在两个文件里就报错**。指针不会被误报 —— 指针短，够不到判据长度。
  ...
  **两类刻意的例外**，P8 把它们单独列出来而不算失败：
  ...
  | [../RELEASE-NOTES.md](../RELEASE-NOTES.md) 用英文重述内部结论 | 它是**对外**的、必须自成一体，读它的人打不开 `docs/`。这是一条边界，不是漂移 —— 但**改了行为就要同步它**，而这一步没有自动化，只能靠收尾时人工过一遍 |
...
- **提到一个已经不存在的文件时，不加反引号也不加链接。** 反引号的意思是“去看这个”，而它不在那里。这条也是为了让用例 **P9**（`$TOOL:TestTool/tools/check_doc_paths.py`）能把“历史提及”和“真断链”分开 —— 它把文档里每一个 markdown 链接、`$BOOT/`… 路径、以及 `docs/…` 反引号路径逐个 resolve，**指不到东西就报错**。这次重组弄断了 105 处，那 105 处全是手工找出来的。
