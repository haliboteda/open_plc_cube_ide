...
| 组 | 用例 | 覆盖 |
|---|---|---|
| TCP 会话 | T1 / T1b / T2 / T3 / T4 | 空闲踢人、不该踢时不踢、拒绝第二连接、传输中不打断、关闭后可重连 |
| 签名 | S1 | 无效签名被拒（上传时 + 下次启动时两半） |
| UDP 发现 | N1 / N2 / N3 / N4 / N5 | 关键词应答、多轮稳定、回复延迟、长时间浸泡、泛洪封顶 |

**反向用例和正向用例一样重要**（T1b「不该踢时没踢」、T4「拒绝状态没卡死」）。它们防的是只在现场暴露的毛病，而且最容易在测试里被漏掉。
