...
**编号 `ISS-` 前缀是 2026-08-22 加的。** 原来是裸 `A1`/`B4`/`C2`，和 `$PROD/docs/STATUS.md` 的需求号 `A1`–`F5` 完全撞车 —— 说"A7 过了"字面上有四个意思。字母数字保留没变，这样旧提交信息里的编号还查得到。全部编号的登记表在 `$PROD/docs/ID-MAP.md`。
...
| **P2** | [ISS-F2](#iss-f2--i2c3--i2c4-完全没有引出) | I2C3 / I2C4 完全没有引出 | 板级测试第 8、9 项**永久无法测试** | 要硬件定（可能要改板） |
| **P3** | [ISS-F1](#iss-f1--knx_rx-pa10-空闲时是-high应该是-low) | KNX_RX(PA10) 空闲时是 HIGH | **KNX 整个收发链**，`TestCase/KNX/` 全部用例 | 等硬件工程师 |
| **P3** | [ISS-F4](#iss-f4--ai--ao-的量程依赖八个焊接跳线默认状态查不到) | AI/AO 量程依赖 8 个焊接跳线，默认状态查不到 | 第 3、4 项的**判据** | 目视看一次板子 |
| P4 | [ISS-F3](#iss-f3--aout_ef-的电阻方向疑似画反标志读不出来) | AOUT_EF 电阻方向疑似画反 | 无（只是读不到 XTR111 故障标志） | 要硬件确认 |
| P4 | [ISS-F5](#iss-f5--spi2_miso-在双焊盘球上spi6-无片选) | SPI2_MISO 在双焊盘球上；SPI6 无片选 | 第 5、6 项的完整功能 | 我们（SYSCFG）+ 硬件（NSS） |
...
## ISS-F1 · KNX_RX(PA10) 空闲时是 HIGH，应该是 LOW

**等硬件工程师。板子侧不通，固件这边没有可做的事 —— 代码保持现状，不要重测。**

| | |
|---|---|
| **是什么** | 总线供电且没人说话时，`KNX_RX` = PA10 实测为 **HIGH** |
| **正确态是什么** | **LOW。** STKNX pin 23 空闲为低 → 点亮 U13 的 LED → 开集输出把 PA10 拉低。签名表在 [../../TestCase/KNX/knx_test.h](../../TestCase/KNX/knx_test.h)：`bus powered, idle` = PH12 HIGH / PD7 HIGH / **PA10 LOW** |
| **实测日期** | 2026-08-26 |
| **在哪找** | 判据 `TestCase/KNX/knx_test.h` 的 Bus power 一节；开机自检打印这一行的代码在 `TestCase/KNX/knx_test.c` 的 `knx_bus_print()` |
| **可能的影响** | ⚠️ **KNX 收发链整体不可用。** PA10 恒高 = 接收链什么都产生不出来，`TestCase/KNX/` 的所有阶段（P0–P5 和 receive-only 模式）都没有意义。**对 bootloader 本身零影响** —— KNX 只在 `v0.1.3-testcase` 分支的板级测试里，产品路径不碰它 |

**为什么可以确定是硬件不是固件**：PA10 的高电平只可能来自 MCU 内部上拉（`/KNX_RX` 板上**没有**上拉，不同于另外三个光耦的 R92/R93/R94）。空闲时它是高的，说明 U13 的开集输出没有在拉低 —— 即 STKNX pin 23 没有输出低，或者 U13 那一路没通。固件唯一能做的就是配上拉，而它已经配了。

**下一步（硬件侧，按这个顺序量）**：

1. `knx_test.c` 开机就打的 `Upper Deck presence` 那一行 —— 如果 PD7/PH12 报 **FLOATING**，那是 Upper Deck 没插好或者它的 3V3 没来，U13 的输出级同样没电，**先修这个，别看别的**
2. C19 对 KNX-（VCCCORE，期望 3.3 V）—— 它经 R89 喂 U13 的 LED
3. R89 两端电压（期望约 1.8 V，即约 5.4 mA）。3.3 V 在而 R89 两端为 0 → STKNX pin 23 是高的（接收器没看到总线），或者 U13 坏
4. 示波 STKNX pin 23 / U13 pin 3 —— 接收器输出到底动不动

这四步和它们各自的含义，`knx_test.c` 的诊断块 `[2]` 已经会自己打出来。

⚠️ **另外两条软件永远看不见的，一并手工量掉**：C27（CVDDHV，装的是 220 µF/10 V）和 C20（CREF，470 nF/10 V），STKNX datasheet Table 2 要求 **≥ 35 V**。任一读数超过 10 V 就是器件过压，必须换。这两条和本条可能是同一个根因。

## ISS-F2 · I2C3 / I2C4 完全没有引出

**2026-08-27 查网表发现。要硬件定：产品上到底要不要这两条。**

| | |
|---|---|
| **是什么** | I2C3（PH7/PH8/PH9）和 I2C4（PB8/PD13/PD11）在 **Bridge J2 ↔ UpperDeck J8** 的对插面上就断了 |
| **证据** | Bridge 原理图 p5 把 `I2C3-*` / `I2C4-*` 画在 J2 连接器符号块上；而 `Production/UpperDeck/netlist.ipc:390-419` 里 J8 的 pin 1、4、11–18、22、23、24、28、29 全是 `NET-(J8-PIN_11-PAD11)` 这种**单管脚未连接网络**。全仓 grep `I2C` 在 UpperDeck / JunctionLink / LowerDeck 三份网表里**只命中 `I2C_2_*`** |
| **可能的影响** | **没有端子、没有插座、没有测试点** —— 信号到不了板外。板级测试第 8、9 项**永久无法测试**（只能做引脚级 GPIO 翻转回读）。对 bootloader 零影响，它不用 I2C |
| **要做什么** | 问硬件：产品需求里要不要 I2C3/I2C4？要的话这是个缺口，得改板。不要的话从需求和测试清单里删掉 |

判据和引脚表在 [../test/BOARD-BRINGUP-CASES.md](../test/BOARD-BRINGUP-CASES.md) §10。

## ISS-F3 · AOUT_EF 的电阻方向疑似画反，标志读不出来

| | |
|---|---|
| **是什么** | XTR111 的错误标志到 MCU 的网络：`~EF ──10k(R79)── PI4 ──1k(R81)── +3V3`，通道 2 是 R80/R82 → PE3 |
| **为什么可疑** | 如果 `~EF` 是开漏（XTR111 数据手册的行为），断言低电平时 MCU 侧只能被拉到 **3.3 × 10/11 ≈ 3.0 V**，读不成逻辑低。而 PI4/PE3 都没有 ADC 功能，所以连模拟读都不行 —— **永远报"无故障"** |
| **在哪找** | `Production/UpperDeck/netlist.ipc:415-416`（J8-26/27）、`:820-828`（R79–R82）、`:874, 858`（U1-8/U2-8）。阻值 `bom.csv:28`（10k）和 `:29`（1k） |
| **可能的影响** | 🟢 **功能上为零** —— 模拟输出本身照常工作（使能脚 OD 经 10k 硬接地，本来就没有软件控制）。影响只是**拿不到故障回读** |
| **要做什么** | 问硬件：R79/R81 和 R80/R82 是不是应该互换？在此之前测试用例照读照打，但标注"此读数不可信" |

## ISS-F4 · AI / AO 的量程依赖八个焊接跳线，默认状态查不到

**只能目视看一次板子。看完把结果记进 [../design/HARDWARE-FACTS.md](../design/HARDWARE-FACTS.md)。**

| | |
|---|---|
| **是什么** | Upper Deck 上八个焊接跳线：**JP1、JP2、JP3、JP4**（模拟输出侧）和 **JP5、JP6、JP8、JP9**（模拟输入侧） |
| **它们决定什么** | **AI**：电压档（68k/22k6，端子电压 = ADC × 4.0089）还是电流档（124 Ω 取样）。**AO**：JP3/JP4 开路时 `Iout = V(PA4) × 10 / 1024`；桥接时 VIN 变成求和节点 `0.8008×V(PA4) + 0.1992×V(VREF)`，成为 4–20 mA 活零点网络，**期望值全变** |
| **为什么查不到** | 焊接跳线**不在 BOM、不在 `positions.csv`**（没有贴片记录），原理图也没印封装变体（`SolderJumper_2_Open` vs `_Bridged`） |
| **可能的影响** | ⚠️ **第 3、4 项的判据不成立** —— 读数能打，但"对不对"无从判断 |
| **附带问题** | `/VREF` 这个网络的来源和电压也查不到（从 Bridge 经 J7-17 进来，Bridge p5 上找不到）。JP3/JP4 桥接的话必须知道它 |

⚠️ **注电压分辨不出 AI 的档位** —— 电压档 0.2494 和电流档 0.2490 两个比例几乎一样。只能目视，或者量端子到 ADC 引脚的电阻。

## ISS-F5 · SPI2_MISO 在双焊盘球上；SPI6 无片选

| | |
|---|---|
| **是什么** | ① **SPI2_MISO = PC2_C**（ball M4），是 UFBGA176 的双焊盘球。数字的 PC2（SPI2_MISO，AF5）只能经 **SYSCFG 模拟开关**到达，**本工程没有任何代码做这件事**。② **SPI6 全板找不到 NSS** —— Bridge 原理图、三份网表、Junction Link J4 引脚表都没有 |
| **在哪找** | ① GPIO 表 xlsx 行 65 自己写着 `(analog switch to PC2; ADC3_INP2)`；`core:variants/STM32H7xx/H743/variant_PLC_H743.h:436` 也标了 `PIN_SPI_MISO PC2_C /* dual-pad */`。② 全仓搜 `SPI6_NSS` 无结果 |
| **可能的影响** | ① SPI2 收不到数据，而且这条路**本工程从未验证过** —— 是第 5 项最大的未知项。② 单从机场景可以不用片选（从机 CS 接地），多从机就必须指定一个普通 GPIO 当软件片选 |
| **要做什么** | ① 固件里调 `HAL_SYSCFG_AnalogSwitchConfig()`，实测验证。② 问硬件：SPI6 无 NSS 是刻意的还是漏了？ |

⚠️ 另一条：**SPI6_MISO = PB4 = NJTRST**，要覆盖它的复用功能。本板调试只用 SWD（Bridge J7 是 10 脚 SWD 座），所以安全。
...
- **做完就删掉，不要打勾留着。** 2026-08-22 清理时删掉了 6 个已完成条目（原 A1 / B3 / C2 / C3 / D2 / D3），连它们的 `<details>` 存档共约 300 行 —— 那些内容的结论已经在 `$PROD/docs/STATUS.md` 和各自的 `M*.md` 里，留在这里只是噪音。
