...
**这里是所有实测数字的唯一出处。** 别处（[../STATUS.md](../STATUS.md)、`RELEASE-NOTES.md`、给硬件的报告）都只引用，不重述。
...
>
> **所以这份表的每一行都带日期，同一个用例跑了两次就两行。** [../STATUS.md](../STATUS.md) 的「最近结果」取最新那行。

**这里只放"测出来是什么"。** 要什么、证明了哪条需求、现在是什么状态 → [../STATUS.md](../STATUS.md)。为什么这么设计 → [../design/](../design/)。还欠什么 → [../work/ISSUES.md](../work/ISSUES.md)。
...
| 2026-08-17 | **E3** bootloader 镜像尺寸 | **96,944 B** / 上限 122,880 B，余量 **21%** |
| **2026-08-22** | **E3** bootloader 镜像尺寸 | **102,328 B** / 上限 122,880 B（120K，尾部 8K 划给 owner 记录区），余量 **20,552 B = 16.7%**。0 errors 0 warnings；`text 101936 · data 384 · bss 209723` |

⚠️ **涨了 5,384 B，全部来自 `Core/Src/sdram_diag.c`**（2026-08-21 为查 D1 线加的四项测量，703 行）。**那次加完从来没有人量过尺寸** —— 08-17 那个 96,944 就这么在总表里挂了五天，而 `Debug/` 里的 `.bin` 已经不是它了。

⚠️ **这两个数字不是同一种东西。** 96,944 是产品代码；102,328 是产品代码 **加上一块为查 D1 线加的诊断代码**。

**所以 16.7% 不能当成 E3 的设计余量** —— 把测试引入的开销算进设计预算，会让下一个人以为产品已经吃掉了那 5KB。

⚠️ **不含诊断的当前数字没测过。** 要拿到它得把 `sdram_diag.c` 从构建里去掉再编一次，而那是改构建 —— **没经同意不动**。`sdram_diag.c` 是临时的：D1 修好之后它要么删、要么进 `#ifdef`，那时这两个数字自然又合成一个。
