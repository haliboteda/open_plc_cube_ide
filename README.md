...
h- using Cube IDE, select Run -> open_plc_main_cdc , (If it is empty, click 'Run Configurations...' -> select 'open_plc_main_cdc' -> 'Run' )  
...
---
**NOTE**  
1- After download and the device restarts, you would see the selection menu on the serial monitor. Send "1" to the plc, with RS232 port (COM5 | /dev/ttyUSB0) in 5 seconds.  
...
d- after finishing the package download, select the openplc board "OPEN-PLC" and select the serial port "COM3 | /dev/ttyACM0"  
