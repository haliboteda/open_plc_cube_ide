...
/* Vendor file copied in for the standalone CAN bring-up test in this same
   TestCase folder (this project's Core/stm32h7xx_hal_conf.h never enabled
   HAL_FDCAN_MODULE_ENABLED, and we don't want to touch CubeMX-owned files) -
   define it here, before stm32h7xx_hal.h is pulled in, so this file compiles
   without any project-wide (-D) or Core/ change.
   Source: STM32Cube_FW_H7_V1.12.1, which ships HAL driver v1.11.5 - the exact
   version in Drivers/ here. Do not take this file from the Arduino core repo:
   that one is v1.11.1. */
