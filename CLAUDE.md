...
| `TestCase/` | ADC / PWM / RS232 / SD / SDRAM 的板级测试入口 + 一份 vendored FatFs。⚠️ **工程里唯一自带一份 HAL 副本的地方**（ADC / SD / SDMMC，靠 `testcase_hal_guard.h` 与工程的 `stm32h7xx_hal_conf.h` 隔离）—— 它不是产品路径，改 HAL 行为之前先读那个守卫头 |
